...
<body class="wide">
	...
	<script>
		...
		kakao.maps.event.addListener(m${status.count}, 'click', function(evt) {
			...
			$('#map > div > div > div > div').css('opacity','.3');
		...
		kakao.maps.event.addListner(map, 'click',function(evt){
		...
		kakao.maps.event.addListner(map, 'dragend',function(evt){
			
			//alert();
			
			//현재 출력되는 지도의 영역을 반환
			//$('#result').text(map.getBounds());
			//$('#result').text(map.getBounds().getSouthWest());
